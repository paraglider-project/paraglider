...
func TestMulticloud(t *testing.T) {
	...
	azureConnectivityCheck1, err := azure.RunPingConnectivityCheck(ctx, azureSubscriptionId, azureResourceGroupName, azureVm1Name, "default", gcpVmIpAddress)
	require.Nil(t, err)
	...
	// Run Azure connectivity check (ping from Azure VM to GCP VM)
	azureConnectivityCheck2, err := azure.RunPingConnectivityCheck(ctx, azureSubscriptionId, azureResourceGroupName, azureVm2Name, "default", gcpVmIpAddress)
	require.Nil(t, err)
...
func TestMulticloudIBMAzure(t *testing.T) {
	...
	if isAzureNetworkWatcherDeployed {
		...
		azureConnectivityCheck1, err := azure.RunPingConnectivityCheck(ctx, azureSubscriptionId, azureResourceGroupName, azureVmName, azureNamespace, ibmVmIpAddress)
		require.Nil(t, err)
		require.True(t, azureConnectivityCheck1)
	}
